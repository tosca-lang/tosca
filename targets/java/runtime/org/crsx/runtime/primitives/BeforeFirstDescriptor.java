...
/**
 * $[BeforeFirst, #string, #separator[, #fallback]] is the constructor named as #string 
 * except all characters from the first occurrence of #separator are omitted.
 *	
 * Default falback is empty string
 *
 * @author Lionel Villard
 */
public class BeforeFirstDescriptor extends ConstructionDescriptor
{
	@Override
	...
	public boolean step(Sink sink, Term term)
	{
		...
		assert term.arity() == 3 || term.arity() == 4 : "$[BeforeFirst, #string, #separator[, fallback]] : missing argument.";
		...
		if (index == -1)
		{
			...
			sink.literal(string.symbol().substring(0, index));
